...
(function (root) {
  ...
  function render(state, route, load={status:'initial'}) {
    ...
    if(route.tab==='overview') {
      ...
      body=`<div class="pdos-grid"><div>${section('Da seguire',`<div class="pdos-next"><p class="pdos-eyebrow">${esc(action.label)}</p><h3>${esc(action.text)}</h3>${button(action.cta,action.action,action.id,'primary')}</div>`)}${section('Persone',owner+tenants+(!m.activeContracts.length?'<p class="pdos-note">Nessun contratto con stato attivo nei dati caricati.</p>':''))}</div><div>${section('L’immobile',`<dl class="pdos-facts pdos-property-facts">${detail('Indirizzo',p.address)}${detail('Zona',p.zone)}${detail('Superficie',p.sqm!=null&&p.sqm!==''?p.sqm+' m²':null)}${detail('Locali',p.rooms)}${detail('Bagni',p.bathrooms)}${detail('Piano',p.floor)}${detail('Tipologia',({apartment:'Appartamento',studio:'Monolocale',room:'Stanza',house:'Casa',loft:'Loft'})[p.propertyType] || p.propertyType)}</dl>`)}${section('Strumenti',`<div class="pdos-tool-list"><a class="pdos-button" href="/inventario?p=${encodeURIComponent(p.id)}" target="_blank" rel="noopener">Inventario <span aria-hidden="true">↗</span></a>${button('Valutazione immobile','valuation',p.id)}${button('Invita il proprietario','owner-invite',p.id)}${p.ownerId?`<a class="pdos-button" href="/proprietario?as=${encodeURIComponent(p.ownerId)}" target="_blank" rel="noopener">Vista proprietario <span aria-hidden="true">↗</span></a>`:''}<details class="pdos-manage"><summary>Gestisci immobile</summary>${button('Elimina immobile','delete',p.id,'danger')}</details>${safeURL(p.youtubeUrl)?`<a class="pdos-button" href="${esc(safeURL(p.youtubeUrl))}" target="_blank" rel="noopener">Video dell’immobile ↗</a>`:''}</div>`)}</div></div>${p.notes?section('Note dell’immobile',`<p class="pdos-description">${esc(p.notes)}</p>`):''}`;
  ...
  function configure(value) {
    ...
    root.document.addEventListener('click', event => {
      ...
      else if(kind==='owner-invite' && m.property.id===id) root.BOOM_OWNER_INVITE?.open(id);
    });
  }
