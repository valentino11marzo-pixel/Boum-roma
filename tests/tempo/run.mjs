...
ok(/function tchip[\s\S]{0,520}pinExact/.test(apt), 'catalogo: origine di ZONA → ≈ anche sulla camminata del chip (boom-geo decide)');
ok(apt.includes('#plChips::-webkit-scrollbar'), 'catalogo mobile: chip in una riga scorrevole, mai tre righe');
ok(apt.includes('Rents ahead') && apt.includes('wlring') && !apt.includes('◌ Waitlist'), 'waitlist = RENTS AHEAD: anello + standby-ticket, via il pill anonimo');
ok(src('apartment-detail.html').includes('WLRING') && src('skyline.html').includes('.p.wl'), 'lo stesso marchio waitlist su scheda e skyline — un marchio, una forma');
