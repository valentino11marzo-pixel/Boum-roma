...
test('continuous monitoring never claims a daily quota even with a legacy status', () => {
  const o = options(); Object.assign(o.monitoring, { mode: 'continuous', usedToday: 5000, dailyCap: null, status: 'working' });
  assert.equal(step(o, 'recheck').state, 'planned');
  assert.match(step(o, 'recheck').detail, /priorità della coda/);
  o.monitoring.status = 'daily_cap';
  assert.equal(step(o, 'recheck').state, 'unavailable');
  assert.doesNotMatch(step(o, 'recheck').detail, /limite giornaliero/);
});
for (const [name, edit] of [
