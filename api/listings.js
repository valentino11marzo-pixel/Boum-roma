...
// hubs, homepage featured) reads `listings` from Firestore DIRECTLY in the
// browser with no auth. That works only while the security rules allow public
// reads. If those rules are tightened (or Firestore test-mode expires), every
// listing page dies with PERMISSION_DENIED and shows a bare "Error".
//
// This endpoint is the safety net. It reads server-side and ALWAYS tries the
// most permissive path that works:
//   1. unauthenticated REST read (fast — succeeds when rules are public)
//   2. admin-authenticated read (the same creds the cron already uses —
//      succeeds when rules are locked to authenticated/admin users)
// The result is cached at the edge so a broken-rules state never hammers
// Firestore. Output shape matches what the client pages expect from a snapshot.
//
...
// Firestore REST value → plain JS value (mirrors api/listing.js).
function fv(v) {
...
// Returns null if creds are not configured so callers can degrade gracefully.
...
export default async function handler(req, res) {
  ...
  // Edge-cache so a broken-rules state is read at most a few times per region.
