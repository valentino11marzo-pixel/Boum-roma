...
export async function finalizeContract(contract){
  ...
  try {
    const existing = await fsList('deadlines', {
      filter: { field: 'linkedContractId', op: 'EQUAL', value: contract.id },
      limit: 300,
    });
    // Per ogni titolo si tiene UNA copia — e se l'operatore ne ha già
    // lavorata una (status non-pending), si tiene QUELLA: cancellare il
    // lavoro fatto per tenere un doppione vergine sarebbe il danno inverso.
    ...
    if (dupes.length) {
      ...
      console.warn(`[finalize] dedupe scadenze ${contract.id}: rimossi ${gone.filter(r => r.status === 'fulfilled').length}/${dupes.length} doppioni auto-generati`);
    }
