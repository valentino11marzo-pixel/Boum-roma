...
function queueSnapshot(rows, now, scope, quietMs) {
  ...
  // A row inside the quiet window is pending (the monitor must show it) but
  // not eligible: it is prepared once the burst is over.
  const quiet = pending.filter(row => !delayed.includes(row) && PRIORITY.quiet(row, now, quietMs));
  ...
  return { scope, openCases: rows.length, currentProposals: rows.filter(task => decisionCurrent(task) && !reviewCurrent(task)).length,
    pending: pending.length, eligible: pending.length - delayed.length - quiet.length, quiet: quiet.length, awaitingReview: review.length,
...
export async function prepareNextCase({ now = Date.now() } = {}) {
  ...
  // settings/segretaria.prepareQuietMinutes (default 3, 0-30): an impossible
  // value falls back to the default and is reported, never adjusted.
  const quietMs = SEG.mergeConfig(config).cfg.prepareQuietMinutes * 60000;
  ...
  const rows = list.rows.filter(task => task.source === 'segretaria'), queueBefore = queueSnapshot(rows, now, scope, quietMs);
  const candidates = rows.map(task => ({ task, reason: pendingReason(task, now) })).filter(row => row.reason)
    ...
    .filter(row => !PRIORITY.quiet(row, now, quietMs));
  ...
  const queue = queueSnapshot(rows.filter(task => task.status === 'open' && task.followUp?.open === true), now, scope, quietMs);
  const out = { enabled: true, prepared, cached, checked: results.length, ...last, errors, results, quietMinutes: quietMs / 60000,
