...
| `owners.html` | La pagina pubblica dei PROPRIETARI (`/owners`, IT) — V1 «LA PIANTA CHE SI ALZA» (23/09/2026, studio `STUDIO_PROPRIETARI_2026-09.md`). «Non ti chiediamo fiducia. Ti diamo le carte.»: la pianta tecnica di un 70 m² d'esempio a Prati (C40) è immagine firma, indice e chiusura; in ogni stanza un PDF VERO fatto dai builder di produzione e timbrato «ESEMPIO — dati inventati» (`carte/`, da `design/owners/genera-fascicolo.mjs`), la riga che conta trascritta identica. Due luci restano spente e la pagina le dice per prime: la serratura (riavere la casa dipende dalla legge) e la cassaforte (una garanzia sui canoni oggi non c'è). La pianta si alza SOLO su un gesto (bottone, cartiglio zona+metri, tocco su una stanza), proiezione ortografica calcolata alla build (`design/owners/pianta.mjs`, runtime `js/owners-pianta.js`); `getAnimations()` vuoto al load. Prezzi da UNA copia (`js/owner-offer.js` + `ASPI_DEFAULTS` + catalogo); il cancello P0-canone (`OFFER.canone.verificato`) tiene fuori ogni tetto del concordato finché ASPI non conferma la tabella delle zone; giorni di riversamento, polizza, referente, recesso e ★ escono SOLO quando il campo è scritto. `owners.html` si scrive a mano, i marcatori li riempie `node design/owners/costruisci-owners.mjs` (idempotente, `--check`). Modulo → `/api/partners/submit` `kind:'owner'` (basta il telefono; `leadType:'landlord'`). Test: `node tests/owners/run.mjs` + `node tests/owners/ui.mjs`, e `npm test -- seo scroll telefono testata media growth`. **Revisione avversaria (24/09)**: zona e metri restano sul telefono (URL non riscritto, `owners_calcolo` senza zona); la tendina si sfoglia da tastiera senza alzare la pianta; i lead proprietario/ente fuori da Richiamo e ricerca rovesciata (`vetoRichiamo`/`vetoFor`); il template «Mandato di Gestione» stampa `mandatoRighe()` e non ha più compenso di default; il PDF della proposta non disegna la firma di un locatore che non ha accettato; nessun sovrapprezzo carta/SEPA in pagina. I punti legali aperti (firma FEA per le locazioni, divieto di sovrapprezzo, modulo di recesso, terms/privacy) stanno in `STUDIO_PROPRIETARI_2026-09.md` §7. |
...
**Il documento per i proprietari che chiedono una valutazione** (9/09/2026
— «la scheda calcolo brandizzata boom diventa un documento per i
proprietari»). Il modulo 1:1 va ad ARPE; al proprietario va QUESTO: pagina 1
il parere di mercato, **pagina 2 la scheda di calcolo brandizzata BOOM**
(`drawSchedaBoom`, esportata): superficie convenzionale riga per riga con
i coefficienti, i 20 parametri con quelli che l'immobile ha davvero, la
subfascia con la soglia che la decide, il canone base (valore × mq), le
maggiorazioni A–H applicate, la regola del tetto quando scatta, e i due
numeri affiancati — **massimo attestabile** dell'accordo e **canone
proposto** — con la frase che li mette in relazione («rientra: attestabile
come concordato, cedolare al 10%» oppure «supera di €X: è un canone di
MERCATO, non attestabile»). Gli STESSI fatti del modulo (`schedaFacts` +
`resolveCanoneInput`, una copia sola: dotazioni REALI di immobile e
annuncio, override persistiti): la scheda del proprietario e il modulo ARPE
non possono divergere. Ciò che manca si scrive «da completare» (zona, mq,
catasto), il massimo diventa «non calcolabile» con l'elenco di cosa manca
— mai un numero. **Si genera anche SENZA contratto**: bottone
**💶 Valutazione** nel modale dell'immobile (admin) →
`openValutazione(null, undefined, {propertyId})` → `POST {propertyId,
canone}`; il documento si ricorda sull'immobile (`valutazioneBoomUrl/At/
Canone`) e, quando c'è, sul contratto. Testata e piede dal marchio vero
(`_pdfbrand`: `masthead` + `stampFooters`), come inventario e verbale — la
testata scritta a mano è sparita. La risposta porta `scheda: {gaps, fascia,
cMax, fits, nP, sc}` e il portal lo dice nel toast («scheda da completare:
catasto»). Test: `node tests/mandato/run.mjs` §8.
...
si firma mai SENZA MANDATO SCRITTO** — quella non è una delega, è una firma
falsa, e la riga è scritta nel pannello, non lasciata al buonsenso.

### Il mandato a firmare del conduttore (9/09/2026 — «devo poter firmare io per il cliente»)
Il caso vero: il cliente ha accettato e firmato la proposta, pensa sia
tutto fatto, e l'email col link del contratto resta chiusa per settimane —
mentre i termini del contratto sono ESATTAMENTE quelli della proposta.
Prima l'unica via era rincorrerlo; il pannello diceva (giustamente) che al
posto del conduttore non si firma. Ora il conduttore può conferire a BOOM
un **mandato con rappresentanza** (art. 1703 ss. c.c.) a sottoscrivere il
contratto in suo nome — e siccome il contenuto è predeterminato dalla
proposta, il doppio ruolo dell'agenzia non fa conflitto (art. 1395 c.c.,
dichiarato nel testo). Le regole, tutte verificate per mutazione:
...
- **Chi ha firmato davvero resta scritto ovunque**: `sign.html` mostra il
  ...
  delle firme e il certificato FES stampano sotto il riquadro le due righe
  di `delegateLines()` («Firma per mandato: X» / «per conto di Y - mandato
  del gg/mm/aaaa (ref)» — anche per la delega del locatore, che prima
  taceva); la scheda ARPE porta la firma con «X per conto di Y», «mandato
  del …», e la riga della firma digitale sulla proposta; il Pack aggiunge
  `10_Mandato_conduttore.pdf` solo quando la firma è per mandato.
- **Nel portal**: card «Mandato del conduttore» in 🖊 Firma ora — senza
  mandato dice dove si dà (sulla proposta), con mandato mostra data/
  proposta/documento e il bottone «Firmo io per il conduttore (mandato)» →
  `setMandatoTenant` scrive `tenantDelegate {name, onBehalfOf, basis, at,
  by}` (rifiutato senza mandato o a firma già apposta), poi si apre il link
  del conduttore e si firma. La riga del contratto nella console PA dice
  «🖊 Mandato a firmare ricevuto il …». Un rinnovo NON eredita mandato,
  deleghe, firme per conto né accettazione (atti di QUEL contratto).
- **La scheda si firma con la proposta** (stessa release — «pronta e auto
  firmata dal cliente quando accetta»): `PA_CONSENT_TEXT` dichiara che
  l'accettazione firma ANCHE la scheda di calcolo del canone (Allegato
  2/B) e che le informazioni sull'immobile sono fornite dalle parti;
  submit registra `consent {text, hash, schedaSigned:true}`, convert lo
  porta su `contract.paAcceptance {at, ref, hash, schedaSigned}`, e
  `firmaOf()` in `fascicolo.js` stampa sul modulo — sotto la riga di firma
  di ciascuna parte — la firma grafica del contratto (PNG incorporato) più
  le righe che la qualificano («firmato digitalmente il …», «firma digitale
  sulla proposta BOOM-… del …», delega/mandato). La pagina della proposta
  mostra PRIMA della firma la card «Rent calculation sheet (Allegato 2/B)»
  coi numeri calcolati DAL SERVER sull'immobile collegato
  (`lookup.js` → `schedaSummary`: zona, superficie convenzionale,
  parametri, fascia, massimo, canone e verdetto) e dichiara i buchi
  («Still to complete on the sheet: cadastral data»); `schedaGaps()`
  (zona · mq · catasto · canone) è la stessa lista che la console PA
  segnala nella riga del canone («scheda 2/B: manca il catasto») e che il
  fascicolo riporta — la scheda esce completa perché i buchi si vedono
  PRIMA, non dopo la firma.
- **Il bug della pagina RLI** (stessa release): `fascicolo.js` stampava
  `rent×12` e «L2» solo sugli studenti; ora legge `FIELDS.rliFacts`
...
`isReceiptDocument` richiede anche `type:'receipt'`, lo schema sia dell'archivio generato sia di Smistatore ricevuta_canone. Un paymentId coincidente e un URL HTTPS non trasformano fattura o prova bonifico in ricevuta. La ricerca legacy filtra il tipo, e /casa applica la stessa guardia; documento privo di tipo resta da verificare.
...
- **Freschezza Oggi/Inbox (21/09)**: il browser distingue conversazioni confermate dal server, copia locale, offline ed errore; le informazioni già lette restano visibili. Questo stato non certifica l'ingresso WhatsApp né la preparazione AI.
  ...
  Prove: `segretariafreshui` e `segretarialiveui`, codice UI reale con sole letture simulate, nessuna scrittura, chiamata AI o comunicazione reale.
- **Apertura lead senza seconda chat (21/09)**: dal modal Inbox il lead passa a `POST /api/homie/conversation` con il solo `leadId`; la UI usa il CID verificato restituito dal server e legge quel documento dal server, anche fuori dalle 200 conversazioni recenti.
  Nessun `set/merge` locale degli header o degli accessi per i lead. Conflitto, timeout o lettura fallita mantengono il contatto nel modulo e consentono un nuovo tentativo; nessun ripiego a `conv_lead_*`.
  Solo la chat selezionata verificata resta in memoria durante gli snapshot della lista, per lo stesso utente; stop e logout la rimuovono. Risposte di selezioni o utenti precedenti non aprono un'altra chat.
  Prova: `inboxleadui`, UI e listener reali con soli confini di rete simulati. Nessuna risoluzione o chiamata AI all'apertura Home; percorsi non lead e riferimento di ritorno da Oggi invariati.
...
- **Consegna preparata scaduta (18/09)**: dopo 48h una WhatsApp mai ritirata mostrava revisione ma bloccava per sempre nuove proposte. `_prepare` può dismetterla soltanto con prova della stessa approvazione e protocollo claim verificati, senza tentativi/esiti precedenti.
  Nuova proposta NON approvata e vecchia azione `rejected` con `delivery.expired` si salvano insieme, con CAS su caso, azione e conversazione; prova originaria e payload restano leggibili. Cache, nuovo evento e retry seguono la stessa regola; errore AI o concorrenza non dismettono nulla.
  Azioni legacy, istanti mancanti, claim o esiti incerti richiedono ancora verifica. `_delivery-guard` rilegge il clock dopo il contesto per non ritirare oltre la finestra; nessun reinvio o nuova approvazione automatica.
  Prove `segretariaprepara` e `segretariaconsegna`: soglia, cache, nuovo evento, audit, errore AI, claim/ACK/inbound/recapito concorrenti e mutazioni sui cancelli/CAS.
  Anche un nuovo caso può risolvere la risposta scaduta di un caso già chiuso della STESSA conversazione: legge la prova originale, conserva chiusura/esito/approvazione e vincola quel caso al commit della nuova proposta. L’esclusione prospettica dall’ownership vale solo insieme alla dismissione atomica; altri owner, copertura incompleta, chat automatica e risposte incerte restano veti.
  La raccolta rispetta il budget esistente; rilettura ownership e CAS su vecchio caso/azione, nuovo caso e conversazione proteggono nuove risposte o cambiamenti concorrenti. Prove `segretariaprepara` con close/capture/approve reali, stessa persona in altra chat, più vecchi owner, fallimenti e mutazioni.
  La chiusura manuale non viene modificata; nessun recupero storico o invio nasce da questa risoluzione, e ogni nuova bozza richiede la consueta approvazione.
