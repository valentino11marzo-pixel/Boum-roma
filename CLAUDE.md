...
| `skyline.html` | **Skyline 2.0** (`/skyline`, embed in apartments via `?embed=1`): Roma DI NOTTE — lo stile liberty di OpenFreeMap riletto dal motore puro `js/skyline-engine.js` (`nightStyle`: base scura, l'ORO è solo BOOM; POI/civici/scudetti spenti), edifici 3D bronzo (`render_height`), arrivo cinematografico (alti e piatti → si scende inclinandosi), e la **pellicola** di card in basso sincronizzata due-vie con pin e camera (mai più la caccia al pin). Selezione = pin acceso + fili delle distanze (≤6: i «My places» sempre, Roma riempie) + anello d'oro sotto il palazzo SOLO su pin `exact` (boom-geo). Badge corsia da dispo-engine (closed non sale, illeggibile → «Ask us»), `≈` sui pin di zona. Ponte embed `boomTieni` con la discovery conservato. **Prima il dato, poi la mappa**: senza tiles/CDN le case restano sfogliabili dalla pellicola. Test: `node tests/skyline/run.mjs`. |
| `board.html` | Il tabellone Solari (`/board`): fotografia di build + idrante Firestore (flippa solo le celle cambiate, grammatica in `js/kiosk-engine.js`). Ogni riga con id è una PORTA verso `/listing/:id` (click/Enter); `?kiosk=1` = vetrina vera (cursor:none). Builder: `design/pages-deco/costruisci-kiosk.py` usa board.html COME template (marker). Test: `node tests/kiosk/run.mjs`. |
...
- **Tests**: `npm test` runs every suite (`tests/run-all.mjs`); `npm test -- money safari`
  ...
  | `tests/kiosk/run.mjs` | il tabellone `/board` (kiosk Solari): ogni glifo emesso esiste nel rullo (mai una cella muta — l'apostrofo di CONCA D'ORO), la zona si ABBREVIA e non si mozza («V. VENETO», mai «VITTORIO VENE»), il prezzo non si corrompe in silenzio (mai «€10,00» per €10.000), le corsie di dispo-engine valgono anche sul tabellone (affittata col solo testo residuo NON sale, data illeggibile → ASK mai NOW), e le giunzioni: il blocco Solari in board.html è byte-identico a solari-engine.html, il builder python usa board.html come template |
  | `tests/og/run.mjs` | l'immagine social si verifica CONTRO IL REPO: i default di `scripts/seo-config.js` risolvono a file esistenti, e le pagine bonificate (board, `apartments-in/*`) non citano mai un'immagine boomrome.com che il repo non contiene — il vecchio default `BOOMsocialprofile.png` non è MAI esistito, e ogni condivisione usciva con l'anteprima rotta |
  | `tests/skyline/run.mjs` | Skyline 2.0: la rilettura notturna (`nightStyle`) non rompe MAI uno stile (garbage compreso) e spegne il rumore (POI/civici/scudetti); la geometria è deterministica; i fili sono ≤6 coi «My places» mai tagliati; la corsia arriva da dispo INIETTATO (closed non si prenota, illeggibile → «Ask us» mai «Available now»); giunzioni: ponte embed `boomTieni` con controllo origin, Street View solo exact/street, anello d'oro solo exact, pellicola PRIMA della mappa (senza tiles la pagina resta utile) |
