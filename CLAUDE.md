...
### Fatturazione elettronica (`js/invoice-engine.js` + Fatture nel portal)
In Italia una fattura NON è un PDF: è l'XML FatturaPA che viaggia sullo SdI
(obbligatorio B2B/B2C dal 2019, tutti i forfettari dal 2024). Il portale
stampava PDF e ricevute di pigione — nessuno dei due è una fattura, e sotto
c'erano TRE percorsi di creazione con TRE schemi diversi sulla stessa
collezione `invoices`: uno numerava con `S.invoices.length + 1` (numero che si
RIPETE appena si cancella un documento), la "Fattura Rapida" non scriveva
`number` affatto (righe "undefined", PDF `BOOM_Fattura_undefined.pdf`), e
`autoInvoiceForPayment` FATTURAVA IL CANONE — soldi che passano
dall'inquilino al proprietario, contati come ricavo BOOM. `parseInt(amount)`
mangiava i centesimi.
- **`js/invoice-engine.js`** — motore PURO (`window.BOOM_INVOICE` +
  `module.exports`), aritmetica in CENTESIMI interi. L'IVA si calcola sul
  totale per aliquota (riepilogo), non riga per riga: sommare le imposte di
  riga dà scarti da 1-2 cent che lo SdI rifiuta. `computeTotals` (bollo €2
  solo senza IVA e solo OLTRE €77,47; ritenuta con aliquota su quota di base —
  23% su 50% = l'11,5% delle provvigioni — e opzionale per riga),
  `nextNumber` (progressivo per anno E sezionale, letto dal MASSIMO emesso;
  le bozze non consumano numero), `validate` (checksum P.IVA/CF, codice
  destinatario 7 char vs `0000000` privato vs `XXXXXXX` estero, Natura
  obbligatoria a IVA 0, forfettario che espone IVA = errore), `buildXML`
  (FatturaPA 1.2.2 FPR12 completo di `IscrizioneREA` — obbligatorio per le
  S.r.l. — `DatiRitenuta`, `DatiBollo`, `RiferimentoNormativo`; emoji rimossi,
  `Causale` spezzata sui 200 char invece che troncata, date in ora LOCALE
  perché `toISOString()` daterebbe 31 dicembre una fattura emessa alle 00:30
  del 1° gennaio), `creditNoteFrom` (TD04: una fattura emessa non si cancella,
  si storna).
- **Editor nel portal** (`openInvoiceEditor`): righe con IVA/Natura, totali
  live, validazione che disabilita "Emetti" finché qualcosa non torna,
  anagrafica fiscale del cliente che si RICORDA (`billing` sul doc
  users/clients/landlords — la seconda fattura si compila da sola), PDF di
...
- **`billing/company`** (collezione NUOVA, admin-only in firestore.rules):
  ...
  in lettura a chiunque (il sito pubblico lo legge) — stessa ragione di
  `/payout`. Impostazioni → Dati di fatturazione.
- **La ricevuta di pigione resta, ma smette di travestirsi**: i documenti
  auto-generati sui canoni nascono `kind:'receipt'`, serie separata `RIC-…`,
  fuori dalla numerazione fiscale e fuori dal KPI "Fatturato". I documenti
  già esistenti sono riclassificati al volo dal flag `autoGenerated`.
- **`COMPANY.piva` era `17546591000`, un refuso che non superava il checksum**
  ufficiale a 11 cifre — ed era stampata su ogni PDF prodotto dal portale. La
  P.IVA reale di Egidi Immobiliare S.r.l. è **`17322991005`** (verificata,
  pinnata in `tests/invoice/run.mjs` insieme al rifiuto del vecchio valore).
  Precompila il form Impostazioni; la fatturazione elettronica legge comunque
  `billing/company`, unica fonte che ha anche sede e REA — **da salvare lì
  prima di emettere il primo documento**.
- **`COMPANY.iban` era `IT00X0000000000000000000000`** e finiva in OTTO punti
  vivi, fra cui il sollecito WhatsApp all'inquilino: ogni inquilino in ritardo
  riceveva un IBAN inesistente. Ora `payoutIban()` legge `settings/payout`
  (già usata da /casa), poi `billing/company`, e se manca DICE che manca.
- **`downloadContractPDF` era definita due volte**: l'ultima (uno stub da 4
  righe, 2.500 righe più in basso) vinceva e chiamava `generateContractPDF()`
  a secco, scavalcando la guardia firme — su un contratto COMPLETO "scarica"
