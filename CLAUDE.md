...
| `owners.html` | La pagina pubblica dei PROPRIETARI (`/owners`, IT), rifatta il 23/09/2026 sulla grammatica di `/executive` v4: una porta sola (modulo «Valuta la tua casa» → `/api/partners/submit` `kind:'owner'`, dove per il proprietario **basta il telefono** e il lead nasce `leadType:'landlord'` con la zona), prova subito, fascicolo dei documenti, conto del concordato, costi, «in breve», FAQ con FAQPage derivato dalle `<details>` visibili. Regola: ogni frase ha un file dietro — la vecchia pagina prometteva una «garanzia di solvibilità nel mandato» che il mandato generato dal portale non contiene, un portale «in tempo reale» con dati finti, supporto 24/7 e statistiche senza fonte. Mandato di revisione per un secondo agente: `docs/PROMPT_CODEX_OWNERS.md`. Test: `npm test -- seo scroll telefono growth`. |
