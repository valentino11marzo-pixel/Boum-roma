...
- **Tests**: `npm test` runs every suite (`tests/run-all.mjs`); `npm test -- money safari`
  ...
  | `tests/documents/smista.mjs` | lo Smistatore dalle porte (PR #234/#235): un `docId` non archivia due volte e il doppione non paga modello né Storage (anche a budget zero), uno sconosciuto MAI sotto un immobile, un proprietario SOLO fra i suoi — letti **per id** (il tetto dei 200 del catalogo non riguarda una relazione), lista vuota = nessun candidato e mai il catalogo libero, oltre 50 immobili/contratti → `unknown` DICHIARATO (`relationDegraded`), e con `budget` (l'oggetto di `runBudget`) il modello non parte senza tempo e `storageUpload` ferma i retry che non può finire, uscendo con l'errore vero (503) invece del kill a 60s |
