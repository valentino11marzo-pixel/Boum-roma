#!/usr/bin/env python3
# IL COLLOQUIO 5.0 — la stessa sostanza (sette obiezioni vere, sette
...
.cam-casa { position:relative; overflow:hidden;
...
/* ══ 5.0 · LA MESSA IN SCENA ══════════════════════════════════════════ */
/* il numerale in filigrana DIETRO la domanda */
.a3-testa { position:relative; padding-top:clamp(30px,4vw,58px); }
.a3-testa .a3-num { position:absolute; top:0; left:-.04em;
  line-height:.8; font-size:clamp(84px,14vw,190px);
  color:rgba(255,215,0,.065); z-index:0; }
.atto3.inv .a3-testa .a3-num { left:auto; right:-.04em; }
.a3-tema, .a3-q, .a3-chi { position:relative; z-index:1; }
.a3-tema { display:inline-block; margin-bottom:4px; }
.a3-q { margin-top:10px; font-size:clamp(27px,4.2vw,56px);
  line-height:1.08; }
/* la domanda si SVELA nel verso dello zigzag; risposta e colpo entrano
   sfalsati. Solo con moto pieno: reduced-motion resta immobile. */
@media (prefers-reduced-motion:no-preference){
  html.vivo .a3-q { clip-path:inset(-6% 102% -6% -2%);
    transition:clip-path 1.05s cubic-bezier(.22,1,.36,1) .12s; }
  html.vivo .atto3.inv .a3-q { clip-path:inset(-6% -2% -6% 102%); }
  html.vivo .a3-testa.dentro .a3-q
    { clip-path:inset(-6% -2% -6% -2%); }
  html.vivo .atto3 .a3-risposta.sale { transition-delay:.18s; }
  html.vivo .atto3 .a3-colpo.sale { transition-delay:.34s; }
}
/* la scia: atto chiuso, prossima domanda */
.atto-fine { margin-top:clamp(28px,3.6vw,44px); display:flex;
  align-items:center; gap:12px; font-size:10px; font-weight:700;
  letter-spacing:.22em; text-transform:uppercase; color:var(--text-3); }
.atto-fine::before { content:'✓'; color:var(--gold); letter-spacing:0; }
.atto-fine i { font-style:normal; font-family:var(--display);
  font-weight:250; font-size:14.5px; letter-spacing:0;
  text-transform:none; color:var(--text-2); }
.atto-fine::after { content:''; flex:1; height:1px;
  background:var(--line-0); }
/* il registro finale: le sette obiezioni, sbarrate una a una */
.reg-lista { list-style:none; margin:clamp(18px,2.4vw,26px) 0 0;
  padding:0; display:flex; flex-wrap:wrap; gap:10px 30px; }
.reg-lista li { position:relative; font-family:var(--display);
  font-weight:250; font-size:clamp(15px,1.6vw,19px);
  letter-spacing:-.01em; color:var(--text-3); padding:3px 0; }
.reg-lista li::after { content:''; position:absolute; left:-2%;
  right:-2%; top:56%; height:1.5px; background:var(--gold);
  opacity:.75; transform-origin:0 50%; }
@media (prefers-reduced-motion:no-preference){
html.vivo .reg-lista li::after { transform:scaleX(0); }
html.vivo .registroF.colpite .reg-lista li::after { transform:scaleX(1);
  transition:transform .5s cubic-bezier(.3,1,.4,1); }
}
html.vivo .registroF.colpite .reg-lista li:nth-child(1)::after { transition-delay:.15s; }
html.vivo .registroF.colpite .reg-lista li:nth-child(2)::after { transition-delay:.27s; }
html.vivo .registroF.colpite .reg-lista li:nth-child(3)::after { transition-delay:.39s; }
html.vivo .registroF.colpite .reg-lista li:nth-child(4)::after { transition-delay:.51s; }
html.vivo .registroF.colpite .reg-lista li:nth-child(5)::after { transition-delay:.63s; }
html.vivo .registroF.colpite .reg-lista li:nth-child(6)::after { transition-delay:.75s; }
html.vivo .registroF.colpite .reg-lista li:nth-child(7)::after { transition-delay:.87s; }
.reg-fine { margin-top:clamp(22px,3vw,34px);
  font-family:var(--display); font-weight:220;
  font-size:clamp(26px,3.6vw,46px); line-height:1.1;
  letter-spacing:-.015em; }
.registroF .btn-primary { margin-top:clamp(18px,2.4vw,26px); }
.registroF .imbarco-garanzia { margin-top:14px; display:flex; gap:8px;
...
def atto(idx, sid, dark, inv, tema, chi, q, lead, det, prove, cifra,
    ...
    fine = ''
    if prossima:
        fine = ('\n    <p class="atto-fine sale">Answered'
                f'<i>Next \u2014 \u201c{prossima}\u201d</i></p>')
    v = ' inv' if inv else ''
    chips = ''
    if prove:
...
<section class="section{d} atto3{v}" id="{sid}">
  <div class="container">{intro}
    <div class="a3-testa sale">
      <span class="a3-num" aria-hidden="true">0{idx}</span>
      <span class="a3-tema">{tema}</span>
      <p class="a3-q">{q}</p>
...
A1 = atto(1, 'dubbio', True, False, 'The rule',
  ...
  prossima='Is Rome really that brutal?')
...
A2 = atto(2, 'roma', False, True, 'The market',
  ...
  SOLARI + '\n\n    ' + CONTO,
  prossima='I can search Idealista myself — for free.')
...
A3 = atto(3, 'solo', True, False, 'The machine',
  ...
  FINDER + '\n\n    ' + CAMMINATA,
  prossima='How do I know you are real?')
...
A4 = atto(4, 'chi', False, True, 'The counterpart', 'You — as you should',
  ...
  prossima='I am not even in Italy yet.')
...
A5 = atto(5, 'estero', True, False, 'The distance',
  ...
  prossima='And if you find nothing I like?')
...
A6 = atto(6, 'garanzia', False, True, 'The guarantee',
  ...
  'The clause, verbatim', """<div class="clausola sale">
    ...
    <a class="clausola-vai sale" href="/terms.html" target="_blank"
      ...
      them →</a>""",
  prossima='OK — what actually happens the minute I pay?')
...
# ── il registro finale: le sette obiezioni sbarrate ─────────────────────
STRIP_A = """    <div class="imbarco-strip sale">
...
uno(s, STRIP_A)
s = s.replace(STRIP_A, """    <div class="registroF sale">
      <span class="eyebrow"><i></i>The interrogation — complete</span>
      <ul class="reg-lista" aria-label="Your seven objections, answered">
        <li>Why pay €350 upfront?</li>
        <li>Is Rome really that brutal?</li>
        <li>Can't I do it myself, free?</li>
        <li>Are you even real?</li>
        <li>I'm not in Italy yet.</li>
        <li>What if you find nothing?</li>
        <li>What happens when I pay?</li>
      </ul>
      <h3 class="reg-fine">Nothing left standing.<br>
        <span class="hl">Five lines — and we hunt.</span></h3>
...
<script>
/* Il registro finale: le sette obiezioni si sbarrano una a una */
(function () {
  'use strict';
  var reg = document.querySelector('.registroF');
  if (!reg) return;
  if (matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  if (!('IntersectionObserver' in window)) return;
  new IntersectionObserver(function (v, o) {
    if (!v.some(function (x) { return x.isIntersecting; })) return;
    o.disconnect();
    reg.classList.add('colpite');
    try { gtag('event', 'pfs_interrogation_complete'); } catch (e) {}
...
for ago in ('id="dubbio"', 'id="roma"', 'id="solo"', 'id="chi"',
            ...
            'class="registroF sale"'):
    uno(s, ago, OUT)
assert s.count('class="atto-fine sale"') == 6, 'sei scie'
