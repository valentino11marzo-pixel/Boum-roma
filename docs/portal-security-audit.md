...
- ~~**One credential is exposed.** The `parse-docs` bearer token is stored as a plain string in Firestore doc `config/parse_docs` (`boom_doc_parser.html:512`). With wide-open rules, any signed-in user can read it and burn through your Anthropic budget.~~ **RISOLTO (23/08/2026)**: `/api/parse-docs` accetta l'ID token dell'admin (ruolo riletto server-side) e la pagina non legge più `config/parse_docs` — nel browser non scende alcun segreto. Il doc non era comunque mai stato creato: lo strumento era fermo su "Parser config missing".
